...
public class ProductDao {
    ...
    public Product findProductByID_redis(Long productId) throws BusinessException {
        logger.debug("findProductByID_redis: productId = {}", productId);
        String key = String.format("product:%d", productId); // Redis key
        Product product;
        ...
        try {
            ...
            if (this.redisUtil.hasKey(key)) {
                // 缓存命中，从Redis获取
                product = (Product) this.redisUtil.get(key);
                logger.debug("findProductByID_redis: Cache hit, product = {}", product);
            } else {
                // 缓存未命中，从数据库查询
                ProductPoExample example = new ProductPoExample();
                ProductPoExample.Criteria criteria = example.createCriteria();
                ...
                // 将BO对象缓存到Redis
                this.redisUtil.set(key, (Serializable) product, 3600L); // 设定缓存时间为1小时
                logger.debug("findProductByID_redis: Cache miss, fetched from DB and saved to cache, product = {}", product);
            }
        } catch (Exception e) {
            logger.error("findProductByID_redis: Error occurred", e);
            throw new BusinessException(ReturnNo.INTERNAL_SERVER_ERR, "内部错误");
        }
